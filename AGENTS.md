...
`stop_after_phase`, `resumed_from`, `resumed_into`, `resume_count`, `plan_revisions`, `replan_pending`, `overrides`, `abandoned_pending_tree`, `abandoned_pending_head`, `activation_generation`, `finish_requested`. `STATE_VERSION` 3 adds four more — `round_history`, `transient_failures`, `retry_not_before`, `clarifications`. Several carry an invariant beyond "a new field":
