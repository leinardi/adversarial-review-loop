...
- **`clarify_history` is evidence like `round_history`** — carried forward, never reset; **only `commands.clarify` appends**, only with `SUPERSEDES` lines that each name exactly one finding of the round answered (`reviewer.valid_retractions` at an `oscillation.ordinal_of` ordinal), only under the fingerprint guard and never while that phase's `active_review` lease is live. Its one reader is `_prior_rounds_section`; it never touches a verdict, `round_history`, or either stall signal. → `state-fields`
