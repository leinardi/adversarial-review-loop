...
# ARL_STATE_DIR -- so each shard is the same script running a disjoint subset.
#
...
# the end of the run, not the instant they happen; `ARL_SELFTEST_JOBS=1` restores the
...
LOGS=$(mktemp -d "${TMPDIR:-/tmp}/arl-selftest-logs.XXXXXX")
trap 'rm -rf "$LOGS"' EXIT

pids=()
for ((shard = 0; shard < JOBS; shard++)); do
	ARL_SELFTEST_SHARD="$shard/$JOBS" "$SELFTEST" "$@" >"$LOGS/$shard.log" 2>&1 &
	pids+=("$!")
done
