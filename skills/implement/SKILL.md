...
- If a blocking finding is ambiguous, or contradicts an earlier round of the same phase, ask the reviewer **before** guessing: `${CLAUDE_PLUGIN_ROOT}/scripts/arl.sh clarify --question "…"`. It is one prose question against the review that just ran — no new commit attempt, no new round, and it changes no state you depend on. A wrong guess costs a whole round; a question costs one of a small allowance, and the denial itself tells you how many are left. Prefer it over re-running the same fix twice. If the answer shows the finding was simply wrong, the reviewer can retract it there, and the next round is shown the retraction — so no code change is needed for that finding alone.
