...
3. **If the banner says `--replan` was granted**, redefine only the phases it names (the current one onward — earlier phases are immutable and already committed) by reading the active plan and running `set-phases` with one `--phase` per replacement phase, exactly as the banner's command shows. Every other mutation is denied until that command has run. Then continue into the (possibly renamed) current phase.
   ...
   **In both cases above, how the `set-phases` command is spelled matters.** The gate reads it as shell words and refuses a line break or backslash continuation (put the whole command on a single line, however long), a backtick, and a `$` — anywhere in the command, quoted or not. Write each description as plain prose and name files and symbols in words rather than in code formatting. A refusal names which of the three it was; fix that and re-run with the same real phases.
