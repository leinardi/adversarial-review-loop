...
- Emit `<<<ARL-FINDINGS>>>` and `<<<ARL-END>>>` on their own lines, exactly once each, and exactly one `VERDICT`. Missing markers, a missing `VERDICT`, or a second block fails again — and this time the original failure is what the gate reports.
