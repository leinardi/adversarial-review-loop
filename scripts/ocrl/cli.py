...
#: numbers ``scripts/ocrl.sh`` passes to ``timeout`` and a hard maximum here too. They are
...
#: How many seconds this process has in total before ``scripts/ocrl.sh`` kills it, or ``None``
...
def _hook_deadline(sub: str) -> float | None:
    ...
    ``OCRL_HOOK_DEADLINE_SEC`` is what the shim passes alongside its own ``timeout``, so a
    test that shrinks the shim timeout (``OCRL_SHIM_TIMEOUT_*``) shrinks this in step and the
    ...
    ceiling -- exactly the clamp ``ocrl_bounded_timeout`` applies on the other side. Anything
    else falls back to the ceiling, which is what the shim uses when the override is refused.
    """
    ceiling = _HOOK_CEILINGS.get(sub)
    if ceiling is None:
        return None
    raw = os.environ.get("OCRL_HOOK_DEADLINE_SEC", "")
...
def main(argv: list[str]) -> int:  # noqa: PLR0911, PLR0912 - one return per subcommand, which is the table
    ...
    if sub == "intent":
        from ocrl.commands import intent  # noqa: PLC0415
    ...
    if sub in ("defer", "status", "report", "finish", "deactivate"):
        ...
        return handler(rest)
    if sub == "pause":
        from ocrl.commands import pausecmd  # noqa: PLC0415
