...
`scripts/ocrl/cmdshape.py` uses it to decide the shape of a command that wants to create a
...
**This vendored copy of bashlex was modified on 2026-08-21 by the opencode-review-loop
...
Every `from bashlex import …` was rewritten to `from ocrl._vendor.bashlex import …`:

```bash
sed -i -E 's/^(\s*)from bashlex import /\1from ocrl._vendor.bashlex import /' bashlex/*.py
```
