...
from ocrl import commands, gitsnap, report
...
opencode-review-loop: nothing may be accepted before the phase list is frozen -- there is no \
...
here -- the exit is /opencode-review-loop:resume, or a fresh /opencode-review-loop:implement \
<plan.md>.
"""

_ALL_PHASES_COMMITTED: Final = """\
opencode-review-loop: every frozen phase is already committed -- there is no phase left for an \
acceptance to apply to. Accepting the final review is out of scope for this command: \
/opencode-review-loop:finish reports what a failing final review found and leaves the mode \
armed to keep iterating, or /opencode-review-loop:stop leaves the mode entirely.
"""
...
def _refusal(state: State, config: Config) -> str | None:  # noqa: PLR0911 - one return per status this command must name, matched exactly
    ...
    if status in ("COMPLETE", "DISARMED"):
        return f"opencode-review-loop: nothing is gated in this worktree ({status}), so there is nothing to accept.\n"
    if status == "STALE":
        return (
            f"opencode-review-loop: this activation is past ttl_hours ({config.as_int('ttl_hours')}), so its baseline "
            "can no longer be trusted and what the accepted tree would be a delta from is unknown. Run "
            "/opencode-review-loop:resume first, then accept again.\n"
        )
    if status in ("ARM_FAILED", "RESUMED"):
        return f"opencode-review-loop: this activation is {status}; there is no live activation here to accept anything into.\n"
    return f"opencode-review-loop: cannot accept while the activation is {status}.\n"
...
def _message(outcome: _Outcome) -> str:
    lines = [f"opencode-review-loop: accepted phase {outcome.phase}'s current tree ({outcome.tree}) by user override.\n"]
    ...
    with state.transaction():
        ...
        if current != expected:
            raise commands.Refused(
                f"opencode-review-loop: {hooks.describe_move(expected, current)} while this accept was being "
                f"prepared, so nothing was accepted. The activation is now {current.summary}. Run "
                "/opencode-review-loop:accept again.\n"
            )
...
def run(argv: list[str]) -> int:
    ...
    # close. See `ocrl.commands.hooks.Activation`'s own docstring.
    ...
    try:
        ...
        sys.stdout.write(f"opencode-review-loop: the working state could not be snapshotted ({exc}); nothing was accepted.\n")
        return 1
