...
whole invocation goes through :meth:`ocrl.harness.Harness.review_command`, exactly as
``reviewer.invoke`` does, and what is rendered below is the :class:`ocrl.harness.Command`
...
import ocrl
from ocrl import commands, gitsnap, harness, paths, planrev, reviewer
...
#: so leaves :attr:`ocrl.harness.Command.cwd` unset -- the child then inherits the gate's own.
...
def _compose(
    ...
    *,
    ...
    hard refusal every other command makes of it (``ocrl.harness.UnknownHarness``), and falling
...
def _render(implementation: harness.Harness, command: harness.Command, *, bundle_dir: Path) -> str:
    """One composed :class:`ocrl.harness.Command`, as a developer reads it.
