...
When that line cannot run at all -- a refused sandbox, an unreadable ``ocrl.sh``, a missing
...
ARMING_COMMAND: Final = re.compile(r"\A\s*/opencode-review-loop:(?:implement|resume)(?:\s|\Z)")

BLOCKED: Final = (
    "opencode-review-loop: enforcement was requested but the request could not be recorded ({detail}). "
