...
def describe_change(before: Fingerprint, now: Fingerprint, reason: str) -> str:
    if before[:3] != now[:3]:
        return (
            "opencode-review-loop: this activation was re-armed while completion was pending, "
...
class Completion:
    ...
    def commit(self, *, reviewed: str, reason: str, refuse_if_review_now_required: bool = False) -> None:
        ...
        ``ocrl config ttl_hours ...`` shrinking it during the (possibly minutes-long) review
        ...
        **Residual windows remain, and none of them can be closed here.** Between
        ...
        with state.transaction():
            ...
            # ``ttl_hours``, and a concurrent ``ocrl config ttl_hours ...`` shrinking it during
            ...
            try:
                ...
                raise commands.Refused(
                    f"opencode-review-loop: about to complete, but the working state could not be re-checked first ({exc}), "
                    "so completion is refused. The mode stays armed.\n"
                ) from exc

            if after.tree != reviewed:
                raise commands.Refused(
                    f"opencode-review-loop: tree {reviewed} was about to be recorded as complete, but the worktree is now {after.tree} — "
            ...
            if after.tree != gitsnap.head_tree(repo):
                raise commands.Refused(
                    "opencode-review-loop: about to complete, but the worktree is no longer clean — "
            ...
            if refuse_if_review_now_required:
                ...
                if (
                    ...
                    raise commands.Refused(
                        "opencode-review-loop: about to complete without a review, but the activation's own state no "
                ...
                if requested is not False:
                    if requested is True:
                        raise commands.Refused(
                            "opencode-review-loop: `finish` was requested while this completion was pending, so "
                    ...
                    raise commands.Refused(
                        f"opencode-review-loop: finish_requested is {requested!r}, not the boolean the schema "
                ...
                if config_module.load(repo, overrides=state.data.get("overrides")).as_bool("final_review"):
                    raise commands.Refused(
                        "opencode-review-loop: `final_review` was enabled while this completion was pending, so "
