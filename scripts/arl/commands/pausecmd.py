...
USAGE: Final = (
    "adversarial-review-loop: usage: /adversarial-review-loop:pause [N | 0 | all]\n"
...
adversarial-review-loop: the phase list is not frozen yet, so there is no phase to pause after. \
Nothing was changed.

Freeze the phases first (the arm banner prints the set-phases command), then pause. To arm \
with a target already set, /adversarial-review-loop:implement <plan.md> --until N.
"""

_ALL_PHASES_COMMITTED: Final = """\
adversarial-review-loop: every frozen phase is already committed, so there is no phase left to \
pause after. Nothing was changed.

Ending the turn completes the activation; /adversarial-review-loop:finish reviews the whole \
activation cumulatively first, and /adversarial-review-loop:stop leaves the mode.
"""
...
def _refusal(state: State, config: Config) -> str | None:  # noqa: PLR0911 - one return per status this command must name, matched exactly
    ...
    if status in ("COMPLETE", "DISARMED"):
        return f"adversarial-review-loop: nothing is gated in this worktree ({status}), so there is nothing to pause.\n"
    if status == "STALE":
        return (
            f"adversarial-review-loop: this activation is past ttl_hours ({config.as_int('ttl_hours')}), so it cannot be "
            "continued as it stands and a pause target would decide nothing. Run /adversarial-review-loop:resume --until N, "
            "which refreshes the activation and sets the target in one step.\n"
        )
    if status in ("ARM_FAILED", "RESUMED"):
        return f"adversarial-review-loop: this activation is {status}; there is no live activation here to pause.\n"
    return f"adversarial-review-loop: cannot pause while the activation is {status}.\n"
...
def _apply(state: State, *, config: Config, raw: str) -> str:
    ...
    with state.transaction():
        ...
        if target and target < phase:
            raise commands.Refused(
                f"adversarial-review-loop: phase {target} is already committed (the loop is on phase {phase} of {total}), "
    ...
    if target == 0:
        ...
        return (
            f"adversarial-review-loop: pause target cleared ({was}). The loop will run to the end of the plan; it is on "
            f"phase {phase} of {total}. There is no pause target left, so no turn will end paused.\n"
            f"{_unresolved_note(status)}"
        )
    return (
        f"adversarial-review-loop: pausing after phase {target} of {total}:\n\n    {state.phase_desc(target)}\n{note}\n"
    ...
    **The two are not the same fence, and must not be worded as though they were.**
    ...
    if status == "NEEDS_HUMAN":
        return (
            ...
            "(/adversarial-review-loop:accept, or /adversarial-review-loop:stop to leave the mode).\n"
        )
    if status == "RECONCILE":
        return (
...
def run(argv: list[str]) -> int:
    ...
    try:
        ...
        sys.stderr.write(f"adversarial-review-loop: {exc}\n")
        return 2
