...
component (:func:`arl.atomic.verified_file` -- a symlinked ``bundles/<seq>/`` leaves ordinary
...
_NO_QUESTION: Final = 'adversarial-review-loop: clarify needs a question -- pass one as --question "...".\n'

_NOT_ACTIVE: Final = "adversarial-review-loop: clarify only works while the review loop is ACTIVE (this activation is {status}). There is no live review to ask about.\n"

_PHASES_NOT_FROZEN: Final = (
    "adversarial-review-loop: nothing has been reviewed yet -- the phase list is not frozen. Read the frozen plan and run set-phases first.\n"
)

_REPLAN_PENDING: Final = (
    "adversarial-review-loop: the phase list is being re-planned; there is no settled review to clarify. "
    "Freeze the revised phases with set-phases first.\n"
)

_NO_ROUND: Final = (
    "adversarial-review-loop: no review has run for phase {phase} yet, so there is nothing to clarify. "
    "Attempt the commit and let the reviewer respond first.\n"
)

_BUNDLE_GONE: Final = (
    "adversarial-review-loop: the bundle for phase {phase}'s most recent review (seq {seq}) is no longer on disk, "
    "so there is nothing to attach a question to. If this phase reached a standing disagreement, that belongs in "
    "the reason field of /adversarial-review-loop:accept.\n"
)

_LIMIT: Final = (
    "adversarial-review-loop: {used} clarifications already used for this run (limit {limit}). "
    "Address the review's findings, or if the disagreement is genuine, take it to /adversarial-review-loop:accept.\n"
)

_MOVED: Final = "adversarial-review-loop: the activation changed while the question was being prepared ({change}); nothing was asked.\n"

_MOVED_DURING_RUN: Final = (
    "adversarial-review-loop: the activation changed while the reviewer was answering ({change}); "
...
_SUPERSEDED: Final = (
    "adversarial-review-loop: a newer review (seq {seq}) of phase {phase} completed while the reviewer was answering, "
...
def _bundle_attachments(act_dir: Path, seq: int, digest: str) -> list[tuple[Path, str]] | None:
    ...
    **Read from the manifest ``build_bundle`` wrote, checked against the digest recorded on
    the round that produced it** (:func:`arl.reviewer.bundle_manifest`). The bundle directory
...
def _stage(
    ...
    the bundle's own stable one -- see :func:`arl.reviewer.stage_attachments` for exactly
    ...
    # can be made to cover the delivery. See :class:`arl.harness.Attachment`.
...
def _ask(activation: commands.Activation, question: str) -> str:
    ...
    try:
        prose = reviewer.run_clarify(
            ...
            staged_attachments,
            staged_question,
            prompt_file=arl.prompt_path("reviewer-clarify"),
            title=title,
            out_path=out_path,
            config=config,
        )
