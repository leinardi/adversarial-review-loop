...
if TYPE_CHECKING:  # pragma: no cover - the hot path must not import these to type-check
    ...
    from arl.hookio import HookInput
...
This is an integration fault, not a review finding. Tell the user, and leave the mode with /adversarial-review-loop:stop if it persists.
"""
...
mode with /adversarial-review-loop:stop.

Do not implement the plan: enforcement was requested and is not running.
"""

UNSCOPABLE_INTENT: Final = """\
This session asked for enforcement (/adversarial-review-loop:implement or :resume was submitted), but the request marker {path} {detail}, so the gate cannot tell which repository it was for. Every mutation in this session is denied until it is resolved, and nothing has been recorded against this repository.

Tell the user. /adversarial-review-loop:stop discards the request; they can then arm again from the intended repository.
"""
...
UNBOUND: Final = """\
This worktree is armed by adversarial-review-loop (activation {session}, status {status}), but this session is not bound to it: neither /adversarial-review-loop:implement nor /adversarial-review-loop:resume ran in this Claude Code process.

Every mutation and commit here is denied until the loop is bound to this session. Tell the user: /adversarial-review-loop:resume continues the plan in this session with every approval kept; /adversarial-review-loop:stop leaves the mode.
...
Run /adversarial-review-loop:implement <plan.md> again, or /adversarial-review-loop:stop to leave the mode.
"""
...
Stop and tell the user. They can run /adversarial-review-loop:accept to approve the current tree \
without another review and continue, or leave the mode with /adversarial-review-loop:stop.
"""
...
Continue in {successor}, or re-arm this worktree from scratch with /adversarial-review-loop:implement <plan.md>.
"""
...
SET_PHASES_ALLOWED_REPLAN: Final = "adversarial-review-loop: set-phases is the one command allowed while a replan is pending."
...
adversarial-review-loop: a newer review of phase {phase} finished while this approval was being \
...
Stop here and tell the user. /adversarial-review-loop:accept approves the current tree without \
another review and continues; /adversarial-review-loop:stop leaves the mode. The gate keeps \
...
def _guard_state_root(hook: Hook, payload: HookInput) -> None:
    ...
    # `ARL_STATE_DIR`, which never matches an absolute target at all. `realpath` handles a
...
def _prepare(hook: Hook, *, state: State, config: Config, repo: str, command: str) -> Snapshot:
    ...
    """
    from arl import cmdshape, gitsnap  # noqa: PLC0415 - reached only by a commit
...
def _checked_base_tree(hook: Hook, state: State) -> str:
    ...
    """
    from arl import gitsnap  # noqa: PLC0415 - reached only by a commit, never on the hot path
...
def _gate_commit(hook: Hook, *, state: State, config: Config, repo: str, command: str) -> None:
    ...
    if tree == base:
        approve("adversarial-review-loop: the working state is byte-identical to the last approved tree, so no review was needed. Commit allowed.")
    if state.tree_approved(tree):
        approve(f"adversarial-review-loop: this exact tree ({tree}) was already approved by a previous review. Commit allowed.")
    ...
    if not diff.stdout:
        approve("adversarial-review-loop: no content difference from the last approved tree. Commit allowed.")

    if gitsnap.all_paths_ignored(repo, base, tree, config):
        approve("adversarial-review-loop: every changed path matches ignore_globs, so the review was skipped. Commit allowed.")
...
def _escalate(hook: Hook, *, state: State, config: Config, expected: hooks.Activation, reason: str) -> None:
    ...
    ``/adversarial-review-loop:stop`` while a review runs, and an escalation landing after that
...
def _guard_reset(hook: Hook, *, repo: str, command: str) -> None:
    ...
    *succeeded*, so every shape that made it raise was passed through instead -- including
    ...
    """
    from arl import cmdshape, gitsnap  # noqa: PLC0415 - reached only by a reset
