...
from arl.atomic import ensure_private_dir, read_verified_file
from arl.config import Config
from arl.harness import Attachment, Captured, CaptureSpec, ClarifySpec, Command, PayloadError, ReviewSpec, TranscriptError, Usage, model
...
#: OpenCode's is -- see :data:`arl.harness.opencode.SESSION_ID_RE`, which documents the live
...
def _session_argv(spec: ReviewSpec) -> list[str]:
    ...
    :class:`~arl.harness.ReviewSpec` never carries both, and a fresh run always carries an id
...
class AssignedSessions:
    ...
    def capture(self, spec: CaptureSpec) -> Captured:
        ...
        Never raises: a miss is a log line and a falsy :class:`~arl.harness.Captured`, which
