...
Everything here was previously inline in :mod:`arl.reviewer` and is moved verbatim --
...
from arl.atomic import FILE_MODE, ensure_private_dir
from arl.config import Config
from arl.harness import Captured, CaptureSpec, ClarifySpec, Command, ReviewSpec, Usage, model
...
def _list_sessions(*, repo: str, config: Config, act_dir: Path, seq: str) -> list[Any] | None:
    ...
    ``ARL_SESSION_LIST_CMD`` is the test seam, parallel to ``ARL_REVIEWER_CMD``: a stand-in
    ...
    """
    from arl.reviewer import run_bounded  # noqa: PLC0415 - the gate imports this module at module scope; a top-level import back would be a cycle

    reviewer_cmd = os.environ.get("ARL_REVIEWER_CMD", "")
    session_list_cmd = os.environ.get("ARL_SESSION_LIST_CMD", "")
...
class OpenCodeHarness:
    ...
    def review_command(self, spec: ReviewSpec) -> Command:
        ...
        return Command(
            argv=[
                ...
                # them -- see :class:`arl.harness.Attachment`, and `reviewer.invoke`'s
    ...
    def transcript(self, raw: bytes) -> bytes:
        ...
        :func:`arl.reviewer.invoke` has already checked by the time this is called.
        """
        return raw
