...
#: Which fallback a crashing entrypoint owes its event. Mirrors ``ARL_FAILCLOSED``.
FailClosed = Literal["none", "pretool", "stop", "posttool"]

DENY_REASON: Final = (
    "adversarial-review-loop: internal gate error ({detail}). Denying to fail closed. "
    "Run /adversarial-review-loop:status, then /adversarial-review-loop:stop if the mode is wedged."
)

STOP_BLOCK_REASON: Final = (
    "adversarial-review-loop: internal error in the Stop gate ({detail}). The final review did not run. Run /adversarial-review-loop:status."
)

POSTTOOL_CONTEXT_REASON: Final = (
    "adversarial-review-loop: internal error in the post-commit check ({detail}). "
    "The commit was NOT confirmed against the approved tree, so this is not a verification. "
    "Run /adversarial-review-loop:status."
)
...
class Hook:
    ...
    One instance per process. The shell used globals (``ARL_DECIDED``, ``ARL_FAILCLOSED``)
    ...
    # terminal decisions, the way the shell's ``arl_deny`` / ``exit 0`` did, instead of
    ...
    def allow(self, reason: str = "adversarial-review-loop: allowed") -> NoReturn:
