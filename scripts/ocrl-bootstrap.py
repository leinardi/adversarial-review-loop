#!/usr/bin/env python3
"""Trusted entrypoint for the opencode-review-loop gate.

Must be invoked by absolute path under ``python3 -I``::

    python3 -I "$PLUGIN_ROOT/scripts/ocrl-bootstrap.py" "$@"

Never ``python3 -m ocrl``. Hooks run with the repository under review as cwd, and ``-m``
puts cwd at ``sys.path[0]`` ahead of ``PYTHONPATH``, so a repository containing
``ocrl/__main__.py`` -- or merely ``json.py`` -- would execute arbitrary code *as the
...
``ocrl`` is imported. Rule 3 -- nothing is written inside the repository under review --
...
# This file lives in <plugin>/scripts/, alongside the ocrl package.
...
def _bytecode_cache_dir() -> str:
    ...
    return os.path.join(base, "opencode-review-loop", "pycache")
...
def _overlaps(a: str, b: str) -> bool:
    ...
    *ancestor* of a source tree still writes into it: with a prefix of ``/``, the bytecode
    for ``/plugin/scripts/ocrl/x.py`` lands at ``/plugin/scripts/ocrl/x.pyc``.
    """
...
def main(argv: list[str]) -> int:
    ...
    # Deliberately not at module scope: importing ocrl before sys.path is established
    ...
    from ocrl.cli import main as cli_main  # noqa: PLC0415
